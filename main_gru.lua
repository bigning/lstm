...
local function gru(x, prev_h)
    local reset_gate_linear_input = nn.Linear(params.rnn_size, params.rnn_size)(x)
    local reset_gate_linear_h = nn.Linear(params.rnn_size, params.rnn_size)(prev_h)
    local reset_gate_linear = nn.CAddTable()({reset_gate_linear_input, reset_gate_linear_h})
    local reset_gate = nn.Sigmoid()(reset_gate_linear)

    local update_gate_linear_input = nn.Linear(params.rnn_size, params.rnn_size)(x)
    local update_gate_linear_h = nn.Linear(params.rnn_size, params.rnn_size)(prev_h)
    local update_gate_linear = nn.CAddTable()({update_gate_linear_input, update_gate_linear_h})
    local update_gate = nn.Sigmoid()(update_gate_linear)
    ...
    local next_h_2_2 = nn.CMulTable()({update_gate, hhat})
    local next_h_2 = nn.CSubTable()({hhat, next_h_2_2})
